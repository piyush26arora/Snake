document.addEventListener('DOMContentLoaded', () => {
    ...
    const scoreDisplay = document.querySelector('span h1')
    const startBtn = document.querySelector('.start')
  
    const width = 10
    let currentIndex = 0        //so first div in our grid
    let appleIndex = 0          //so first div in our grid
    let currentSnake = [2,1,0]  //initial blocks for snake
    ...
    function startGame() {
            ...
            //removes previous elements on the screen and past data like interval and score
      currentSnake.forEach(index => squares[index].classList.remove('snake'))
      squares[appleIndex].classList.remove('apple')
      clearInterval(interval)
      score = 0

      randomApple()  // placing apple for the first time

        // values for first time
      direction = 1
  ...
  //______________________________________________________________
    ...
    function control(e) {
      ...
      if(e.keyCode === 39) {
        ...
        direction = +width                      //if we press down, the snake head will instantly appear in 
                                                //the div ten divs from where you are now
      }
    }
